...
- **현재 버전** — `0.6.0` (지난 버전은 아래 [버전 이력](#버전-이력))
...
| 버전 | 날짜 | 무엇이 바뀌었나 |
|---|---|---|
| `0.6.0` | 2026-09-12 | 녹음 로버스트니스 이식(BEP/LEP) — 마이크 끊김 감지·자동 일시정지, 회전 브레이크·80MB 상한, wakeLock, 완료 idle전환, 복구 배너 정직성 (메인 기능 불변) |
| `0.5.1` | 2026-09-11 | 모바일 자동 번역이 한국어 라벨을 뭉개던 것 차단 (`lang="ko"`·notranslate) |
